...
def highest_reward_missions(request):
    try:
        ...
        id_satisfactory_mission=[item.mision.id for item in satisfactory_mission]
        missions=Mision.objects.filter(id__in=id_satisfactory_mission).order_by('recompensa')
        serializer=MisionSerializer(missions.reverse()[:5],many=True)
        return Response(serializer.data)
    except Exception as e:
        raise e